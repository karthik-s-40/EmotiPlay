...
# YouTube API Credentials (Replace with your actual API Key)
...
def detect_facial_expression():
    ...
    logging.info(f"Captured frame: {frame.shape} pixels")
...
# Route to get Spotify songs based on mood
...
# Route to get YouTube videos based on mood
@app.route('/youtube-videos', methods=['POST'])
def youtube_videos():
    data = request.json
    mood = data.get('mood', 'happy')
    videos, next_page_token = get_youtube_videos(mood)
